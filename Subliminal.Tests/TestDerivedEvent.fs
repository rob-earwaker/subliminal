...
[<Property>]
let ``emits first context value from source observable`` context1 (context2: obj) =
    ...
    test <@ observer.ObservedValues.[0] = context1 @>
    test <@ observer.ObservableCompleted @>
    
[<Property>]
let ``emits context value to observers that subscribe after completion`` context =
    use subject = new Subject<obj>()
    let event = DerivedEvent<obj>.FromObservable(subject)
    ...
    use subscription = event.Subscribe(observer)
    test <@ observer.ObservedValues.Count = 1 @>
    test <@ observer.ObservedValues.[0] = context @>
    test <@ observer.ObservableCompleted @>
    
[<Property>]
let ``does not complete until context value is emitted from source observable`` context =
    use subject = new Subject<obj>()
    let event = DerivedEvent<obj>.FromObservable(subject)
    let observer = TestObserver()
    use subscription = event.Subscribe(observer)
...
[<Property>]
let ``completes immediately for non-empty source observable`` context =
    let observable =
